...
# Tối ưu: Lấy dữ liệu 1 lần duy nhất cho toàn bộ App
def lay_du_lieu():
...
# Lấy dữ liệu sẵn ở đây để các Tab bên dưới không phải gọi lại nữa
df_tong = lay_du_lieu()
...
with tab1:
    ...
    if st.button("Ghi sổ Trà Tắc", use_container_width=True, type="primary"):
        if so_tien > 0:
            ...
            st.rerun()
    hien_thi_lich_su_tab(df_tong, "Trà tắc")
...
with tab2:
    ...
    if st.button("Ghi sổ Giặt Sấy", use_container_width=True, type="primary"):
        if so_tien_gs > 0:
            ...
            st.rerun()
    hien_thi_lich_su_tab(df_tong, "Giặt sấy")
...
with tab3:
    ...
    if st.button("Ghi sổ Sửa Đồ", use_container_width=True, type="primary"):
        if so_tien_sd > 0:
            ...
            st.rerun()
    hien_thi_lich_su_tab(df_tong, "Sửa đồ")

# --- TAB 4: BÁO CÁO ---
with tab4:
    st.header("📈 Tổng Doanh Thu")
    if not df_tong.empty:
        df_tong['Ngày'] = df_tong['Thời Gian'].dt.date
        st.write("---")
        st.subheader("📅 Chọn khoảng thời gian")
        ngay_min, ngay_max = df_tong['Ngày'].min(), df_tong['Ngày'].max()
        ngay_chon = st.date_input("Lọc báo cáo theo ngày:", value=(ngay_min, ngay_max))
        
        if len(ngay_chon) == 2:
            df_loc = df_tong[(df_tong['Ngày'] >= ngay_chon[0]) & (df_tong['Ngày'] <= ngay_chon[1])]
        elif len(ngay_chon) == 1:
            df_loc = df_tong[df_tong['Ngày'] == ngay_chon[0]]
        else:
            df_loc = df_tong.copy()
