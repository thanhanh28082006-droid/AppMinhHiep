...
LINK_GOOGLE_SHEETS = "https://docs.google.com/spreadsheets/d/1rNjsqV3OUNtQeYd4OXAb3oNuPrpPj6jv2wZYvwv9oTw/edit?pli=1&gid=0"
...
# --- TAB 1: TRÀ TẮC (ĐÃ FIX LỖI LOAD LẠI NHIỀU LẦN) ---
with tab1:
    st.header("🥤 Quản lý Trà Tắc")
    ds_mon = {"Chọn Đồ Uống": 0, "Trà tắc (10k)": 10, "Nước cam (20k)": 20, "Trà chanh (15k)": 15}
    
    # Dùng session_state để lưu biến bộ nhớ tạm, chống App bị F5 liên tục
    if 'gia_tien_tt' not in st.session_state:
        st.session_state['gia_tien_tt'] = 0
    if 'ghi_chu_tt' not in st.session_state:
        st.session_state['ghi_chu_tt'] = "Khách lẻ"
    if 'mon_dang_chon' not in st.session_state:
        st.session_state['mon_dang_chon'] = "Chọn Đồ Uống"

    # Hàm ngầm tự cập nhật giá tiền mà không làm tải lại trang
    def cap_nhat_gia():
        mon = st.session_state['mon_dang_chon']
        st.session_state['gia_tien_tt'] = ds_mon[mon]
        st.session_state['ghi_chu_tt'] = "Khách lẻ" if mon == "Chọn Đồ Uống" else mon.split(" (")[0]

    chon_mon = st.selectbox("⚡ Chọn món nhanh:", list(ds_mon.keys()), key='mon_dang_chon', on_change=cap_nhat_gia)
    
    col_a, col_b = st.columns(2)
    with col_a:
        so_tien = st.number_input("Số tiền (k):", min_value=0, step=1, key="gia_tien_tt")
    ...
    with col_b:
        ...
        ten_khach = st.text_input("Ghi chú:", key="ghi_chu_tt")
    ...
    if st.button("Ghi sổ Trà Tắc", use_container_width=True, type="primary"):
        if so_tien > 0:
            ...
            # Sau khi lưu xong, trả các ô về 0 gọn gàng
            st.session_state['gia_tien_tt'] = 0
            st.session_state['ghi_chu_tt'] = "Khách lẻ"
            st.session_state['mon_dang_chon'] = "Chọn Đồ Uống"
            st.rerun()
